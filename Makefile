...
build: build-frontend
	go build -ldflags="-X github.com/asano69/archiver/internal/version.Version=$(VERSION)" -o $(BINARY) ./cmd/$(BINARY)

.PHONY: server
server: 
	#./archiver migrate up --dir=pb_data
...
migrate-collections:
	...
	yes | go run ./cmd/archiver migrate collections
	ls -1 migrations/*.go | sort | head -n -1 | xargs rm -f
