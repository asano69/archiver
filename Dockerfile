...
# "archiver" to match the Go backend's default (see internal/config).
ARG APP_NAME=Archiver
...
RUN --mount=type=cache,target=/go/pkg/mod \
    ...
    CGO_ENABLED=0 go build -trimpath -ldflags="-s -w -X github.com/asano69/archiver/internal/version.Version=${VERSION}" -o archiver ./cmd/archiver
...
RUN addgroup -g 1000 archiver && \
    adduser -D -u 1000 -G archiver archiver

COPY --from=go-builder /build/archiver /usr/local/bin/archiver

RUN mkdir -p /certs /archiver/pb_data
